 ...
 */
public class TraitTickTock extends TraitProgressiveStats {
  ...
  public void onUpdate(ItemStack tool, World world, Entity entity, int itemSlot, boolean isSelected) {
    ...
    if(player.getStatFile().readStat(StatList.PLAY_ONE_MINUTE) % TICK_PER_STAT > 0) {
      return;
    }

    // we don't update if the player is currently breaking a block because that'd reset it
    if(playerIsBreakingBlock(entity) || (entity instanceof EntityPlayerMP && ((EntityPlayerMP) entity).getActiveItemStack() == tool)) {
      return;
    }
    ...
    // attack
    if(player.getStatFile().readStat(StatList.PLAY_ONE_MINUTE) % (TICK_PER_STAT * 3) == 0) {
    ...
    // speed
    else if(player.getStatFile().readStat(StatList.PLAY_ONE_MINUTE) % (TICK_PER_STAT * 2) == 0) {
