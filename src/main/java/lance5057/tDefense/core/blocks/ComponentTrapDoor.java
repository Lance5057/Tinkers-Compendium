...
public class ComponentTrapDoor extends Block
{
    public static final PropertyDirection FACING = BlockHorizontal.FACING;
    public static final PropertyBool OPEN = PropertyBool.create("open");
    public static final PropertyEnum<ComponentTrapDoor.DoorHalf> HALF = PropertyEnum.<ComponentTrapDoor.DoorHalf>create("half", ComponentTrapDoor.DoorHalf.class);
    ...
    public ComponentTrapDoor(Material materialIn)
    {
        super(materialIn);
        this.setDefaultState(this.blockState.getBaseState().withProperty(FACING, EnumFacing.NORTH).withProperty(OPEN, Boolean.valueOf(false)).withProperty(HALF, ComponentTrapDoor.DoorHalf.BOTTOM));
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos)
    {
        ...
        else if (state.getValue(HALF) == ComponentTrapDoor.DoorHalf.TOP)
        {
    ...
    public IBlockState getStateForPlacement(World worldIn, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer)
    {
        ...
        if (facing.getAxis().isHorizontal())
        {
            ...
            iblockstate = iblockstate.withProperty(HALF, hitY > 0.5F ? ComponentTrapDoor.DoorHalf.TOP : ComponentTrapDoor.DoorHalf.BOTTOM);
        }
        else
        {
            ...
            iblockstate = iblockstate.withProperty(HALF, facing == EnumFacing.UP ? ComponentTrapDoor.DoorHalf.BOTTOM : ComponentTrapDoor.DoorHalf.TOP);
        }
    ...
    {
        return this.getDefaultState().withProperty(FACING, getFacing(meta)).withProperty(OPEN, Boolean.valueOf((meta & 4) != 0)).withProperty(HALF, (meta & 8) == 0 ? ComponentTrapDoor.DoorHalf.BOTTOM : ComponentTrapDoor.DoorHalf.TOP);
    }
    ...
    public int getMetaFromState(IBlockState state)
    {
        ...
        if (state.getValue(HALF) == ComponentTrapDoor.DoorHalf.TOP)
        {
            i |= 8;
        }
    ...
    {
        return (face == EnumFacing.UP && state.getValue(HALF) == ComponentTrapDoor.DoorHalf.TOP || face == EnumFacing.DOWN && state.getValue(HALF) == ComponentTrapDoor.DoorHalf.BOTTOM) && !((Boolean)state.getValue(OPEN)).booleanValue() ? BlockFaceShape.SOLID : BlockFaceShape.UNDEFINED;
    }
